...
router.post('/', async (req, res) => {
  ...
  for (const item of items) {
    if (item.paninda_id) {
      await supabase.rpc('decrement_stock', { item_id: item.paninda_id, qty: item.qty });
    }
  }

  if (payment === 'utang' && utangName) {
    const note = items.map(i => i.name).join(', ');
    const { data: existing } = await supabase.from('utangs')
      .select('*').ilike('name', utangName).single();
