...
export const addPaninda = async ({ name, price, stock, imgFile }) => {
  let img = ''
  if (imgFile) {
    const filename = `${Date.now()}-${imgFile.name}`
    const { error: uploadError } = await supabase.storage.from('paninda-images').upload(filename, imgFile)
    if (!uploadError) {
      const { data: urlData } = supabase.storage.from('paninda-images').getPublicUrl(filename)
      img = urlData.publicUrl
    }
  }
  const { data, error } = await supabase.from('paninda').insert({ name, price: parseFloat(price), stock: parseInt(stock) || 0, img }).select().single()
  if (error) throw error
  return data
}

export const updatePaninda = async (id, { name, price, stock, imgFile, existingImg }) => {
  let img = existingImg || ''
  if (imgFile) {
    const filename = `${Date.now()}-${imgFile.name}`
    const { error: uploadError } = await supabase.storage.from('paninda-images').upload(filename, imgFile)
  ...
  const { data, error } = await supabase.from('paninda').update({ name, price: parseFloat(price), stock: parseInt(stock) || 0, img }).eq('id', id).select().single()
  if (error) throw error
  return data
}
